...
void Say(struct Point * p){
    printf("Native Point:(%d, %d)", p->x, p->y);
}

struct Point GlobalPoint = {-1, 1};

static JSValue JSSay(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv){
...
static JSValue js_point_get_y(JSContext* ctx, JSValueConst this_val) {
    ...
    if (!p)
        return JS_ThrowTypeError(ctx, "Invalid Point object"); JS_EXCEPTION;
...
void registerNativeObject(JSContext * ctx, JSValue * proto){
    JS_SetPropertyFunctionList(ctx, *proto, js_point_props, countof(js_point_props));
