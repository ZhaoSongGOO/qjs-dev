...
int point_get_x(struct Point* p) { return p->x; }

int point_get_y(struct Point* p) { return p->y; }

void point_set_x(struct Point* p, int x) { p->x = x; }

void point_set_y(struct Point* p, int y) { p->y = y; }

void point_free(struct Point* p) { free(p); }

static JSValue js_point_create(JSContext* ctx, JSValueConst this_val, int argc,
                               JSValueConst* argv) {
  int x, y;
...
static JSValue js_point_free(JSContext* ctx, JSValueConst this_val, int argc,
                             JSValueConst* argv) {
  struct Point* p = JS_GetOpaque(this_val, 1);
...
static void js_point_init(JSContext* ctx) {
  ...
  /*
      point class 是一个函数，名称为XPoint的函数，函数实现是native的
     js_point_create 例如：
      ```
      ...
      js中如果一个函数是构造函数，在使用 new name()的时候,
     会传入函数原型，这里设置传入的原型是 point_proto.
  ...
  */
  ...
  point_class = JS_NewCFunction2(ctx, js_point_create, "XPoint", 2,
  ...
  /*
      这里不可以 free point_class:
     否则会报错，如下。看报错原因是因为尝试对一个引用计数小于等于0的对象进行gc，即重复释放。
      ```
          Assertion failed: (p->ref_count > 0), function gc_decref_child, file
     quickjs.c, line 5751. [1]    35174 abort      ./out/inject_native_type
      ```
      1. 首先我增加了一个函数 JS_GetRefCount 用来打印对象的引用计数，在没有 free
     point_class 的时候，其引用计数为 2。看起来在正常的情况下，qjs vm
     内部需要对这个对象 free 两次。
      2. 继续增加日志，发现在 JS_SetConstructor(ctx, point_class, point_proto);
     后 class 和 proto 引用计数都增加了 1. 看起来这两个互相引用。
      3. 在 JS_SetPropertyStr 执行后，我预期是看到引用计数为 3，
     但是事实上还是 2. 也就是说在 JS_SetPropertyStr 中挂载在 globalobj
     上的对象是通过指针挂载的，而不是对象引用。所以我们不能 free。 4.
